namespace Squirrel.ConsoleApp.Providers.SqlServerScripts
{
    internal class GetObjects
    {
        ...
        public static string GetStoredProcedureDefinitionScript(string storedProcedureName) =>
            @$"SELECT ROUTINE_DEFINITION FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_NAME = '{storedProcedureName}'";
        ...
        public static string GetFunctionDefinitionScript(string functionName) =>
            $"SELECT ROUTINE_DEFINITION FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'FUNCTION' AND ROUTINE_NAME = '{functionName}'";
